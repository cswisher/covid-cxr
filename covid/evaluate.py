...
def evaluate(sess, graph, testfile, testfolder):
    ...
    for i in range(len(testfile)):
        ...
        probs = sess.run(pred_tensor, feed_dict={image_tensor: np.expand_dims(x, axis=0)})[0]
        pred.append(np.array(sess.run(pred_tensor, feed_dict={image_tensor: np.expand_dims(x, axis=0)})).argmax(axis=1))
        print(probs[2])
        prob_normal.append(probs[0])
        prob_pneum.append(probs[1])
        prob_covid.append(probs[2])

    prob_normal = np.array(prob_normal)
    prob_pneum  = np.array(prob_pneum)
    prob_covid  = np.array(prob_covid)
    y_test = np.array(y_test)
    df = pd.DataFrame({'preds': np.squeeze(pred), 
                       'labels':np.squeeze(y_test), 
                       'normal':np.squeeze(prob_normal), 
                       'pneumonia':np.squeeze(prob_pneum), 
                       'covid':np.squeeze(prob_covid)}) 
                        #index=False)
...
if __name__ == '__main__':
    ...
    evaluate(sess, graph, testfile, args.testfolder)
